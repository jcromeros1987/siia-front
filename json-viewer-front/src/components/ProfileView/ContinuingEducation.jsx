// src/components/ProfileView/ContinuingEducation.jsx
import React from 'react';

export default function ContinuingEducation({ items = [] }) {
  return (
      ...
      {items.length > 0 ? (
        items.map((item, i) => (
          <div key={i} className="border-l border-gray-300 pl-3 py-1">
            ...
            <p className="text-[#082C3B] text-xs">{item.año}</p>
          </div>
        ))
      ) : (
        <p className="text-[#082C3B] text-sm italic">No hay formación</p>
      )}
    </div>
  );
}